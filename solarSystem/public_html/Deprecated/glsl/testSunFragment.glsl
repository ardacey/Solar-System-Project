...
precision highp float;
  ...
  // Varying değişkenler
  in vec3 vTexCoord3D; // Vertex shader'dan gelen 3D koordinatlar
  out vec4 fragColor; // Çıktı renk
  ...
  // Permütasyon ve gürültü fonksiyonları
  vec4 permute(vec4 x) {
  ...
  void main(void) {
    ...
    // Renk hesaplaması (spektrodan RGB)
    ...
    float r = float(rbucket1) * (0.0 + i * 4.25) +
